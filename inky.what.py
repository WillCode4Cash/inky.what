...
try:
  f = urllib2.urlopen('http://pi.hole/admin/api.php?topItems=1&auth=*insertyourpiAPIkeyhere*')
  json_string = f.read()
  parsed_json = json.loads(json_string)
...
try:  
  lastblocked = requests.get('http://pi.hole/admin/api.php?recentBlocked&auth=*insertyourpiAPIkeyhere*')
except:
  response = '?'
...
draw.text((130,40), "MY-HOST-NAME", inky_display.RED, datafont)
